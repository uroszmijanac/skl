  ...
  public function __construct ( $meta_box ) {
    ...
    if (isset($meta_box['use_with_theme']))
      if ($meta_box['use_with_theme'] === true){
        $this->SelfPath = get_template_directory_uri() . '/framework/meta';
  ...
  public function show_field_repeater( $field, $meta ) {
    ...
    $meta = get_post_meta($post->ID,$field['id'],true);
      ...
      if (count($meta) > 0 && is_array($meta) ){
        ...
        <span class="re-control"><img src="'.$plugin_path.'/images/remove.png" alt="'.__('Remove','delicious').'" title="'.__('Remove','delicious').'" id="remove-'.$field['id'].'"></span>
    ...
    echo '" alt="'.__('Add','delicious').'" title="'.__('Add','delicious').'" id="add-'.$field['id'].'"><br/></div>';
    ...
    echo '</table><img src="'.$plugin_path.'/images/remove.png" alt="'.__('Remove','delicious').'" title="'.__('Remove','delicious').'" id="remove-'.$field['id'].'"></div>';
...
.at-repater-block{background-color: #FFFFFF;border: 1px solid;margin: 2px; min-height: 50px}
  ...
  public function show_field_color( $field, $meta ) {
    ...
    if( wp_style_is( 'wp-color-picker', 'registered' ) ) { //iris color picker since 3.5
      ...
      echo "<input type='button' class='at-color-select button' rel='{$field['id']}' value='" . __( 'Select a color' ,'delicious') . "'/>";
  ...
  function Validate_upload_file_type($file) {
    if (isset($_POST['uploadeType']) && !empty($_POST['uploadeType']) && isset($_POST['uploadeType']) && $_POST['uploadeType'] == 'my_meta_box'){
      ...
      if (!in_array($ext, (array)$allowed)){
        $file['error'] = __("Sorry, you cannot upload this file type for this field.", 'delicious');
        return $file;
      }
  ...
  public function load_textdomain(){
    ...
    load_textdomain( 'delicious', dirname(__FILE__) . '/lang/' . get_locale() .'mo' );
  }
} // End Class
...
class AT_Meta_Box extends AT_Meta_Box_Big {
	...
	*/
	...
	$meta = get_post_meta($post->ID,$field['id'],true);
	  ...
	  if (count($meta) > 0 && is_array($meta) ){
		...
		if ($field['sortable'])
		  echo '<div class="dt_sortable at_re_sort_handle"></div>';
		
		echo '<span class="re-control"><img src="'.$plugin_path.'/images/remove.png" alt="'.__('Remove','delicious').'" title="'.__('Remove','delicious').'" id="remove-'.$field['id'].'"></span>';
		echo '</table>';
	...
	echo '<span><img src="'.$plugin_path.'/images/remove.png" alt="'.__('Remove me','delicious').'" title="'.__('Remove me','delicious').'" id="remove-'.$field['id'].'" /></span></table></div>';
	$counter = 'countadd_'.$field['id'];
	$js_code = ob_get_clean ();
	$js_code = str_replace("\n","",$js_code);
	$js_code = str_replace("\r","",$js_code);
 ...
 /**
  ...
  public function show_field_image( $field, $meta ) {
    ...
    $w            ='250';
    $h            = '170';		
			$aq_image = aq_resize( $aq_url, $w, $h, true );
    ...
    $multiple     = isset($field['multiple'])? $field['multiple'] : false;
    $multiple     = ($multiple)? "multiFile " : "";
		echo "<span class='simplePanelImagePreview'><img style='width:80%' src='{$aq_image}' ><br/></span>";
    echo "<input type='hidden' name='{$name}[id]' value='{$value['id']}'/>";
    echo "<input type='hidden' name='{$name}[url]' value='{$value['url']}'/>";
    if ($has_image)
      echo "";
