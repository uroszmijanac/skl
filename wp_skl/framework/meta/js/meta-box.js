...
jQuery(document).ready(function($) {
  ...
  if($("select[name='dt_portf_header'] option:selected").val() == 'parallax-title') {
    $("#dt_parallax_bg").show();
  }

    $('select[name="dt_portf_header"]').on("change", function(event){
      var str = '';
      str = $("select[name='dt_portf_header'] option:selected").val();

     if(str == 'parallax-title')
         $("#dt_parallax_bg").fadeIn();
      else
          $("#dt_parallax_bg").hide();
    ...
    // link to link 2
  if($("select[name='dt_team_thumb_icon'] option:selected").val() == 'team_to_link') {
    $("#dt_team_link").show();
  }

    $('select[name="dt_team_thumb_icon"]').on("change", function(event){
      var str = '';
      str = $("select[name='dt_team_thumb_icon'] option:selected").val();
    ...
    // link to link
  if($("select[name='dt_portf_icon'] option:selected").val() == 'link_to_link') {
    $("#dt_portf_link").show();
  }

    $('select[name="dt_portf_icon"]').on("change", function(event){
      var str = '';
      str = $("select[name='dt_portf_icon'] option:selected").val();

     if(str == 'link_to_link')
         $("#dt_portf_link").fadeIn();
      else
          $("#dt_portf_link").hide();
  ...
  if($("select[name='dt_portf_icon'] option:selected").val() == 'lightbox_to_gallery') {
    $("#dt_portf_gallery").show();
  }
    ...
    $('select[name="dt_portf_icon"]').on("change", function(event){
     ...
     if(str == 'lightbox_to_gallery')
         $("#dt_portf_gallery").fadeIn();
      else
          $("#dt_portf_gallery").hide();
       
    });   

    // link to video in lightbox
  if($("select[name='dt_portf_icon'] option:selected").val() == 'lightbox_to_video') {
    $("#dt_portf_video").show();
  }
    ...
    $('select[name="dt_portf_icon"]').on("change", function(event){
     ...
     if(str == 'lightbox_to_video')
         $("#dt_portf_video").fadeIn();
      else
          $("#dt_portf_video").fadeOut();
       
    });      


  if($("#dt_portfolio_navigation input[value='no-filter']:checked").val() == 'no-filter') {
    $("#dt_nav_number").show();
  }  

    $('input[name="dt_portfolio_navigation"]').on("change", function(event){
      var str = '';
      str = $("#dt_portfolio_navigation input[value='no-filter']:checked").val();

     if(str == 'no-filter')
         $("#dt_nav_number").fadeIn();
      else
          $("#dt_nav_number").fadeOut();
	...
	jQuery.fn.extend({
	    ShowStyles: function () {
			...
			subheader_styles['style-1'] = "#dt_bg_type, #dt_subheader_img, #dt_subheader_color";
			subheader_styles['style-2'] = "#dt_bg_type, #dt_subheader_img, #dt_subheader_color";
			subheader_styles['style-3'] = "#dt_bg_type, #dt_subheader_img, #dt_subheader_color";
	...
	if ((a == ('template-portfolio.php')) || (a == ('template-gallery.php')) || (a == ('template-homepage.php')) || (a == ('template-onepage.php')) ) {
		...
		jQuery('#page_sidebars #dt_sidebar_position').hide();
	}
	...
	jQuery.fn.extend({
	    ShowPostFormats: function () {
			...
			var post_formats = {};
			post_formats['post-format-0'] = "#dt_standard_post_custom_fields";
			post_formats['post-format-gallery'] = "#dt_gallery_post_custom_fields";
			post_formats['post-format-link'] = "#dt_link_post_custom_fields";
			post_formats['post-format-quote'] = "#dt_quote_post_custom_fields";
			post_formats['post-format-audio'] = "#dt_audio_post_custom_fields";
			post_formats['post-format-video'] = "#dt_video_post_custom_fields";
	...
	jQuery.fn.extend({
	    ShowPageFields: function () {
			...
			//page templates
			var page_templates = {};
			page_templates['template-blog.php'] = "#dt_blog_options";
			page_templates['template-portfolio.php'] = "#dt_portfolio_options";
			page_templates['template-contact.php'] = "#dt_contact_options";
			page_templates['template-onepage.php'] = "#dt_onepage_options";
